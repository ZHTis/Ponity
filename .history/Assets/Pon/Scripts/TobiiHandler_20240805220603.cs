...
public class TobiiHandler : MonoBehaviour
{
    ...
    IEyeTracker Fourc;
    List<Tobii.Research.EyeData> LefteyeData = new List<Tobii.Research.EyeData>();
    List<Tobii.Research.EyeData> RighteyeData = new List<Tobii.Research.EyeData>();
    float TimeStamp;
    ...
    private IEnumerator LogEyeData(GazeDataEventArgs e) {
        LefteyeData.Add(e.LeftEye);
        RighteyeData.Add(e.RightEye);
        TimeStamp = UnityEngine.Time.time;
        Debug.Log("--------: "+ e.SystemTimeStamp);
        yield return null;
    }
    ...
    void OnDestroy(){
       ...
       File.WriteAllText
        (Application.dataPath + "/Resources/EyeData/" + TimeStamp.ToString("0.00")  + ".json", 
        jsonData);
    }
}
