...
public class TobiiHandler : MonoBehaviour
{
    ...
    void  GazeEventHandler (object sender , GazeDataEventArgs e)
    {
        ...
        perEye.Validity = new Vector4(LeftGaze.Validity.ConvertTo<float>(),
            RightGaze.Validity.ConvertTo<float>(),
            LeftPupilData.Validity.ConvertTo<float>(),
            RightPupilData.Validity.ConvertTo<float>() );
        perEye.TrailTag = TrialTag;
        perEye.FrameTag = FrameTag;
        eyeDataToSave.Add(perEye);Debug.Log("eyeDataToSave: "+ eyeDataToSave.Count);
    }
    ...
    void SaveData(){
        Debug.Log("eyeDataToSave: "+ eyeDataToSave.Count);
