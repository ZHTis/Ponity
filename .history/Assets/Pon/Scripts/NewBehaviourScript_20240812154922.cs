    ...
    IEnumerator ThisIsIt(int how=0)
    {
        ...
        switch(how)//history lept
        {
            ...
            //fully randomized design
                ...
                var combinations =  camIDList.SelectMany(neck => camNeck,(neck,id)=>new {neck,id})
                    .SelectMany(c => targetDistance,(c,d)=>new {c,d})
                    .SelectMany(e => ratio,(e,ratio)=>new {e,ratio});
                foreach (var combination in combinations){Debug.Log(combination);}
