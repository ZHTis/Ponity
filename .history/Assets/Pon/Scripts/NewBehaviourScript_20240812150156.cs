    ...
    IEnumerator ThisIsIt(int how=0)
    {
        ...
        switch(how)//history lept
        {
            ...
            //fully randomized design
                ...
                //ratio
                defineSession(s);
                var combinations_ = camIDList.SelectMany(a => camNeck, (a, b) => new { a, b })
                    .SelectMany(ab => targetDistance, (ab, c) => new { ab.a, ab.b , c})
                    .SelectMany(abc => ratio, (abc, d) => new { abc.a, abc.b, abc.c, d});
                Debug.Log(combinations_.Count() + "/n" + combinations_);

               // if_Shuffle(combinations,true);
