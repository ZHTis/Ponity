...
/// </summary>
   ...
   void FinalBreak(){
       ...
       void TarHandler(GameObject target,float speed){
        if( Input.GetKey("a") ){
  ...
  void Fire(float DestroyTime = 1.1f, bool realFire = true)
    ...
    void targetRandomize(bool debugMode = false)
    {
        if(debugMode == true){
            ...
            if(movingtar!=null){movingtar.transform.position = targetCharacter.location_2;}
        }
        else{
