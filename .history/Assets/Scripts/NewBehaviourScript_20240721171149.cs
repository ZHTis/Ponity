   ...
   void FinalBreak(){
    ...
    void Update()
    {
        ...
        if ((Input.GetKeyDown(KeyCode.A) || Input.GetKeyDown(KeyCode.L)) && keyPressed==false && mainCamera.enabled == true) 
        {
            ...
            behaviorC.touchTimefromPause = Time.time-ponCharacter.kinematicTime;
   ...
   /// <returns></returns>
    IEnumerator ThisIsIt()
    {
        ...
        for(int i = behaviorC.session; i <= sessionLenth; i++)
        {
            ...
            Fire(waitbeforechoice+k);
            timer = 0f;
            yield return new WaitUntil(() => timer > waitbeforechoice+k+0.5 || keyPressed == true);
